<?php
	...
	// Create connection
		...
		if ($action == "getHotelList") {
			...
			$result = mysqli_query($con,"SELECT DISTINCT B.bId, B.uId, H.hName, R.roomNo, C.cId, C.cname, B.bookingDate, B.checkInDate, B.duration, B.guests, B.status FROM booking B, hotel H, room R, category C WHERE H.hId = B.hId AND B.roomNo = R.roomNo AND R.cId = C.cId AND B.status <> 'deleted' ORDER BY B.bId");
			while ($row = mysqli_fetch_array($result)) {
