...
var showHome = function() {
  ...
  if (($.find('#home')).length == 0) {
    ...
    $('#confirmBooking').bind('click', showConfirmBooking);
  ...
  $.ajax({
    ...
    success: function(result) {
      ...
      if (result['status'] == 'ok') {
        ...
        $('#modify-hotel').bind('change', hotels, fillDetailsForHotel);
        $('#update-hotel-button').bind('click', $('#hotel-details'), updateHotel);
        $('#delete-hotel-button').bind('click', $('#remove-hotel').val(), deleteHotel);
      }
    }
  });
...
var fillDetailsForHotel = function(event) {
  var hId = parseInt($(this).children(':selected').attr('id')) - 1;
  $.ajax({
    ...
    success: function(result) {
      ...
      result = JSON.parse(result);
      if (result['status'] == 'ok') {
        var hotelFacilities = result['answer'];
        var facList = $('#mod-fac');
  ...
  $.each(hotelDetails, function(index) {
    $(hotelDetails[index]).val(event.data[hId][index + 1]);
  });
